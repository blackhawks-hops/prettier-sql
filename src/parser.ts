...
export class SQLParser {
  ...
  static tokenize(text: string): string[] {
    ...
    const regex =
      /(SELECT|FROM|WHERE|GROUP BY|ORDER BY|HAVING|JOIN|LEFT JOIN|RIGHT JOIN|INNER JOIN|OUTER JOIN|ON|AND|OR|AS|WITH|UNION|ALL|LIMIT|OFFSET|,|;|\(|\))/gi;
    ...
    while (i < tokens.length) {
      ...
      if (token === "SELECT") {
        ...
        if (i < tokens.length && tokens[i].toUpperCase() === "WHERE") {
          ...
          while (
            i < tokens.length &&
            !["GROUP", "ORDER", "LIMIT", "HAVING"].includes(
   ...
   */
  static parseEmbedded(
    originalParse: (text: string, parsers: any, options: any) => any,
  ) {
